...
def parse_section(driver, url, output_dir):
    ...
    min_i, max_i = INDEXES[os.path.split(output_dir)[1]]
    for index, element in enumerate(elements):
        if min_i < index < max_i:  # Filtering range
            filepath = f"{output_dir}/{index}.png"
...
def main():
    ...
    if args.section == "all":
        for section, url in URLS.items():
            ...
            parse_section(driver, url, output_dir)
    else:
        output_dir = f"screenshots/{args.section}"
        parse_section(driver, URLS[args.section], output_dir)
